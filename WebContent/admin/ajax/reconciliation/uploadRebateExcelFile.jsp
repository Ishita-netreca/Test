...
<%
		...
		try {
			...
			//MyDBUtil.execute(String.format("INSERT INTO `%s`.`tb_reconciliation_update_%s` SET `update_date`=now(), `file`='%s', `user_id`='%s', `type`='3'", db_name, db_name, f.getName().toString(), user_sid));
			int sid = MyDBUtil.getInstance().getInt(String.format("SELECT MAX(sid) AS sid FROM `%s`.`tb_reconciliation_update_%s` WHERE TYPE = 1 ORDER BY `sid` DESC;", db_name, db_name ), "sid");
			int i=0;
			/*
			query2.append(String.format("SELECT * FROM %s.`tb_reconciliation_update_%s` WHERE sid = (SELECT MAX(sid) AS sid FROM %s.`tb_reconciliation_update_%s`)"));
			
			JSONObject jsonObj = MyDBUtil.getJSON(query2.toString(), false);
			
			String year = jsonObj.get("year").toString();
			String month = jsonObj.get("month").toString();
				...
				for (Row row : sheet) {
					...
					if (row.getCell(0) == null || row.getCell(1) == null || row.getCell(2) == null || row.getCell(3) == null) {
						...
						if (!row.getCell(0).toString().contains("Door Code") && !(	row.getCell(0).toString().equals("") || 
							...
							query.append(String.format("INSERT INTO `%s`.`tb_reconciliation_rebate` SET", db_name));

							for (Cell cell : row) {
								if (cell.getColumnIndex() > 15) break;

								strTmp = cell.toString();
								if (cell.getColumnIndex() == 15 || cell.getColumnIndex() == 10) {
