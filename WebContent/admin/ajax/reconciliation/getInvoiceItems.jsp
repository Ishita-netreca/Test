...
<%
		...
		try {
			...
			// 쿼리 입력
			query.append(String.format("SELECT a.*, b.`date` FROM `%s`.`tb_invoice_items_%s` AS a LEFT JOIN `%s`.`tb_invoice_%s` AS b ON a.`invoice_no` = b.`invoice_no` WHERE `b`.`date` BETWEEN STR_TO_DATE('%s','%%m/%%d/%%Y') AND STR_TO_DATE('%s','%%m/%%d/%%Y')", db_name, store_id, db_name, store_id, search_start_date, search_end_date));
