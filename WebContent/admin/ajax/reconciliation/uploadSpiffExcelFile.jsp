...
<%
		...
		try {
			...
			//MyDBUtil.execute(String.format("INSERT INTO `%s`.`tb_reconciliation_update_%s` SET `update_date`=now(), `file`='%s', `user_id`='%s', `type`='4'", db_name, db_name, f.getName().toString(), user_sid));
				...
				for (Row row : sheet) {
					...
					if (row.getCell(0) == null || row.getCell(1) == null || row.getCell(2) == null || row.getCell(3) == null) {
						...
						if (!row.getCell(0).toString().contains("Door Code") && !(	row.getCell(0).toString().equals("") || 
							...
							query.append(String.format("INSERT INTO `%s`.`tb_reconciliation_spiff` SET", db_name));

							for (Cell cell : row) {
								if (cell.getColumnIndex() > 14) break;

								strTmp = cell.toString();
								if (cell.getColumnIndex() == 14 || cell.getColumnIndex() == 9) {
