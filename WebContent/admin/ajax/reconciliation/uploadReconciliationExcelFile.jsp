				...
				for (Row row : sheet) {
					...
					if (row.getCell(0) == null || row.getCell(1) == null || row.getCell(2) == null || row.getCell(3) == null) {
						...
						if (!row.getCell(0).toString().contains("Door Code") && !(	row.getCell(0).toString().equals("") || 
							...
							for (Cell cell : row) {
								if (cell.getColumnIndex() > 20) break;

								strTmp = cell.toString();
								if (cell.getColumnIndex() == 13 || cell.getColumnIndex() == 20) {
									strArrTmp = strTmp.split("/");
									if(strArrTmp.length > 2){	strTmp = strArrTmp[2]+"-"+strArrTmp[0]+"-"+strArrTmp[1]; query.append(String.format(" `%s`=STR_TO_DATE('%s', '%%Y-%%m-%%d'),", columns[cell.getColumnIndex()], strTmp));	}
									else	{	query.append(String.format(" `%s`=null,", columns[cell.getColumnIndex()]));	}
