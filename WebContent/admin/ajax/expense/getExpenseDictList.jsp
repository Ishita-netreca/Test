...
<%
		...
		try {
			...
			// 쿼리 입력
			query.append(String.format("SELECT DATE_FORMAT(`a`.update_date, '%%m/%%d/%%Y %%H:%%i:%%s') as update_date, `a`.*, `b`.`user_id` FROM (SELECT * FROM `%s`.`tb_expense_dict_%s`) AS `a`",db_name,store_id));
