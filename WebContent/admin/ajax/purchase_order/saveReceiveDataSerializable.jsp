		...
		try {
            ...
            for (int i = 0; i < serialIDs.size(); i++) {
                ...
                if (jsonObj.get("serialNo") != null) {
                    ...
                    switch (status) {
                        case 1:
                            query.append(String.format("UPDATE `%s`.`tb_po_items_%s` SET `receive_qty`='1',`status`='1', `received_date`=NOW() WHERE `sid`='%d' AND `po_sid`='%d' AND `item_sid`='%d'; ",
                        ...
                        case 3:
                        	...
                        	*/
                            query.append(String.format("UPDATE `%s`.`tb_po_items_%s` SET  `receive_qty`='1', `serial_no`='%s',`status`='3' WHERE `po_sid`='%d' AND `item_sid`='%d';",
                            		owner_id, storeId, jsonObj.get("serialNo").toString(), poSid, itemSid 
                            ));
                            break;
                        case -44:
