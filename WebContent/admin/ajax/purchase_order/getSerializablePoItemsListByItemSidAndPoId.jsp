...
<%
		...
		try {
		    ...
		    out.print(MyDBUtil.getInstance().getJSONString(String.format("SELECT sid, po_sid as poSid, item_sid as itemSid, serial_no as serialNo, order_qty as orderQty, fulfill_qty as fulfillQty, receive_qty as receiveQty, status FROM `%s`.`tb_po_items_%s` WHERE `po_sid`='%d' AND `item_sid`='%d'", owner_id, storeId, poSid, itemSid), true));
		} catch (Exception e) {
