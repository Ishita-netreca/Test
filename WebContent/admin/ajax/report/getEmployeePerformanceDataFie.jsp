...
<%
	...
	try {
		...
		// 쿼리 입력
		query.append(String.format("SELECT `a`.`year_month`,`d`.`user_id`"));
		...
		query.append(String.format(" SELECT `sid`,`user_id` FROM `wrp`.`tb_user` "));
		query.append(String.format(" ) AS `d` ON `b`.`emp_id`=`d`.`sid`"));
		query.append(String.format(" GROUP BY `emp_id`"));
