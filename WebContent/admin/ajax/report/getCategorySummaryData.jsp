...
<%
		...
		try {
			...
			query.append(String.format("SELECT SUM(`a`.`qty`) AS `qty`,SUM(`a`.`amount`) AS `price`, SUM(`a`.`tax_amount`) AS `tax_amount`, SUM(`a`.`discount`) AS `discount`, SUM(`a`.`subtotal`) AS `subtotal`,`c`.`_category`,IF(`d`.`category` IS NULL, 'etc.', `d`.`category`) AS `category` FROM ("));
			...
			query.append(String.format(" GROUP BY `_category`"));	
