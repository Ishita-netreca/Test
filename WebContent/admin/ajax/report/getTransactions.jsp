...
<%
		...
		try {
		    ...
		    */
		    ...
		    query.append(String.format(" SUM(IF (a.item_type = 0 AND d.`item_type`=0, a.`item_cost`, 0)) AS box_cost,"));
		    query.append(String.format(" SUM(IF (d.item_type IN (2,3), a.`item_cost`, 0)) AS accessory_cost,"));
		    query.append(String.format(" SUM(IF (a.item_type = 3, a.`amount`, 0)) AS payment_cost"));
		    query.append(String.format(" FROM ( SELECT `sid`,`item_type`,`transaction_type`,`item_cost`,DATE_FORMAT(DATE_ADD(`transaction_date`,INTERVAL %s HOUR),'%%Y-%%m') AS `transaction_date`,`item_sid`, i.`amount` FROM `%s`.`tb_invoice_items_%s` as ii, `%s`.`tb_invoice_%s` as i ", timezone_offset, db_name, storeId, db_name, storeId));
		    query.append(String.format(" WHERE ii.`invoice_no` = i.`invoice_no` and(DATE_FORMAT(DATE_ADD(`transaction_date`, INTERVAL %s HOUR), '%%Y-%%m') BETWEEN '%04d-%02d' AND '%04d-%02d')", timezone_offset, beforeYear, beforeMonth,nowYear, nowMonth ));
