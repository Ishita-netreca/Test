...
<%
		...
		try {
		    ...
		    if (categorySid < 0) categorySid = -categorySid;
		    if (subCategorySid < 0) subCategorySid = -subCategorySid;
            ...
            if (categorySid > 0) {
            	query.append(String.format(" AND `category`='%d'", categorySid));
            	if (subCategorySid > 0) {
            		query.append(String.format(" AND `sub_category`='%d'", subCategorySid));
            	}
            }
            ...
            if (categorySid > 0) {
            	query.append(String.format(" WHERE `ab`.`_category` IS NOT NULL"));
            	if (subCategorySid > 0) {
            		query.append(String.format(" AND `ab`.`_sub_category` IS NOT NULL"));
            	}
            }
			...
			while(rs.next()) {
                ...
                sb.append(String.format("\"_category\":%d,", rs.getInt("_category")));
                sb.append(String.format("\"_subCategory\":%d,", rs.getInt("_sub_category")));
