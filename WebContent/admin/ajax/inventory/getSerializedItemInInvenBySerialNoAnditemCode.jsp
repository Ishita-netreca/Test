...
<%
		...
		try {
		    if (storeId == null || serialNo == null || item_code == null || db_name == null) {
		        throw new Exception();
		    }
            ...
            query.append(String.format("SELECT `sid`,`model`,`description`,`sku`,`item_code` FROM `%s`.`tb_item_dict_%s` WHERE `item_code`='%s' AND `sid` IN (SELECT `item_sid` AS `sid` FROM `%s`.`tb_inventory_%s` WHERE `serial_no`='%s' AND `qty` > 0)", db_name, storeId, item_code, db_name, storeId, serialNo));
