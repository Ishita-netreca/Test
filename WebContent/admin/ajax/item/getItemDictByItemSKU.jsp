...
<%
		...
		try {
			...
			query.append(String.format("SELECT `sid`,`item_code`,`model`,`description`,`distributor`,`category`,`sub_category`,`manufacturer`,`color`,`condition`,`a`.`sku`,`item_cost`,`retail_price`,`wholesale_price`,`item_type`,`update_date`,`updater`,`disable`,`serialized`,`carrier_sid`,`vendor_sid`,"));
			query.append(String.format("IF(`a`.`image` IS NOT NULL AND `a`.`image` != '', `a`.`image`, `wrp`.`image`) AS `image` FROM ("));
			query.append(String.format("SELECT * FROM `%s`.`tb_item_dict_%s` WHERE `sku`='%s' ) AS `a`", db_name, storeId, sku));
			query.append(String.format(" LEFT JOIN (SELECT `sku`,`image` FROM `wrp`.`tb_item_dict_[storeid]` WHERE `sku` IN ('%s') ",sku));
