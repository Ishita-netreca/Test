...
<%
        ...
        int transferStatus = MyRequestUtil.getInt(request, "transferStatus", -1);
		...
		try {
			...
			case 1: // Reject
			    query.append(String.format("UPDATE `%s`.`tb_inventory_trans_%s` SET  `status`='%d' WHERE `sid`='%d'", db_name, db_name, transferStatus, transSid));
			    break;
			case 2: // Approval
			    query.append(String.format("UPDATE `%s`.`tb_inventory_trans_%s` SET `appr_date`=NOW(), `appr_user_sid`='%s', `status`='%d' WHERE `sid`='%d'", db_name, db_name, user_sid, transferStatus, transSid));
			    break;
			case 3: // Ship
