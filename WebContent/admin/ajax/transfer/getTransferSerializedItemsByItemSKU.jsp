...
<%
		...
		try {
		    if (sku == null || user_sid == null || db_name == null || currentStatus < 0 || transSid < 1) {
		        throw new Exception();
		    }
            ...
            query.append(String.format("SELECT `sid`,`sku`,`serial_no`,`req_qty`,`appr_qty`,`ship_qty`,`recv_qty` FROM `%s`.`tb_inventory_trans_items_%s` WHERE `trans_sid`='%d' AND `sku`='%s'", db_name, db_name, transSid, sku));
