...
<%
		...
		try {
			...
			if (promotion_sid == 0) {
				query.append(String.format("INSERT INTO `tb_promotion_%s` SET", store_id));
				...
				query.append(String.format("UPDATE `tb_promotion_%s` SET", store_id));
				query.append(String.format(" `description`=?,"));
				mPreparedStatementParams.set(description);
				query.append(String.format(" `start_date`=STR_TO_DATE('%s','%%m/%%d/%%Y'),", start_date));
				query.append(String.format(" `end_date`=STR_TO_DATE('%s','%%m/%%d/%%Y'),", end_date));
			...
			if (promotion_sid > 0) {
				...
				for (int i = 0; i < items.size(); i++) {
					...
					if (items_sid == 0) {
						query.append(String.format("INSERT INTO `tb_promotion_items_%s` SET", store_id));
					} else {
						query.append(String.format("UPDATE `tb_promotion_items_%s` SET", store_id));						
					}
