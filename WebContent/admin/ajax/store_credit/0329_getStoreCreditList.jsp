...
<%
	...
	try {
		...
		// 쿼리 입력
		query.append(String.format("SELECT `sid`,`customer_sid`,`amount`,`total`,`date`,IF(`reason`=1,'Return',IF(`reason`=2,'Check',IF(`reason`=3,'Cash','Others'))) AS `reason` FROM `%s`.`tb_store_credit_%s` WHERE `customer_sid` = '%d' ", owner_id, store_id, customerSid));
