...
<%
		...
		try {
		    if (user_sid == null || db_name == null || permissionGroupSid < 1) {
		        throw new Exception();
		    }
			...
			query.append(String.format("SELECT `sid`,`name`,DATE_FORMAT(`update_date`,'%%m/%%d/%%Y %%H:%%i:%%s') AS `update_date`,`updater` FROM `%s`.`tb_permission_group_%s` WHERE `sid`='%d'", db_name, db_name, permissionGroupSid));
