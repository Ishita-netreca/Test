...
<%
		...
		try {
			...
			// 쿼리 입력
			query.append(String.format("SELECT `confirmation_id`,`dealer_fee` FROM `%s`.`tb_reconsolidation_qpay` WHERE 1=1", db_name));
			if (search_start_date != null && search_end_date != null) {
				query.append(String.format(" AND STR_TO_DATE(`date/time`,'%%m/%%d/%%Y') BETWEEN STR_TO_DATE('%s','%%m/%%d/%%Y') AND STR_TO_DATE('%s','%%m/%%d/%%Y')", search_start_date, search_end_date));
			}
			query.append(String.format(" ORDER BY `date/time`"));
