...
<%
		...
		try {
			...
			if(f.getName().matches(".*CA Con.*")) flag = "consolidation";
			else if(f.getName().matches(".*Qpay.*")) flag = "reconsolidation_qpay";
			else if(f.getName().matches(".*Spiff.*")) flag = "reconsolidation_spiff";
			else if(f.getName().matches(".*Activation_qualified.*")) flag = "reconsolidation_qualified";
			else if(f.getName().matches(".*disqualified.*")) flag = "reconsolidation_disqualified";
			else if(f.getName().matches(".*Rebate.*")) flag = "reconsolidation_rebate";
			sdf = null;
			...
			int max_date = MyDBUtil.getInstance().getInt(String.format("SELECT MAX(date_division) AS sid FROM `%s`.`tb_reconsolidation_bill` ORDER BY `date_division` DESC;", db_name), "sid");
			int min_date = MyDBUtil.getInstance().getInt(String.format("SELECT MIN(date_division) AS sid FROM `%s`.`tb_reconsolidation_bill` ORDER BY `date_division` DESC;", db_name), "sid");
			MyDBUtil.getInstance().execute(String.format("INSERT INTO `%s`.`tb_consolidation_update_%s` SET `update_date`=now(), `file`='%s', `user_id`='%s', `type`='5'", db_name, db_name, f.getName().toString(), user_sid));
			...
			try {
				for (Row row : sheet) {
					...
					if (isHeaderContained) {
						...
						for (Cell cell : row) {
							if(cell.getStringCellValue().toLowerCase().replaceAll("\n","_").replaceAll(" ", "_").replaceAll("__", "_").matches(".*transaction_date.*")){	upload+=1 ;}
							if(cell.getStringCellValue().equals("") || upload < 1) continue;
							strTmp = cell.getStringCellValue();
							if (strTmp != null) {
								columnsMap.put(cell.getColumnIndex(), strTmp.trim().toLowerCase().replaceAll("\n","_").replaceAll(" ", "_").replaceAll("__", "_"));
								isHeaderContained = false;
							}
						}
						
					} else {
						query.append(String.format("INSERT INTO `%s`.`tb_reconsolidation_bill` SET", db_name));
						for (Cell cell : row) {
				...
				query.append(String.format("DELETE FROM `%s`.`tb_reconsolidation_bill` WHERE `update_sid` = %d AND `date_division` BETWEEN %s AND %s;", db_name, sid, min_date, max_date));
				query.append(String.format("DELETE FROM `%s`.`tb_reconsolidation_bill` WHERE `door_name` IS NULL;", db_name));
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (count > 0) {
