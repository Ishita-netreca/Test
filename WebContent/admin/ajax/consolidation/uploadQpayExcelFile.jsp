...
<%
		...
		try {
			...
			int max_date = MyDBUtil.getInstance().getInt(String.format("SELECT MAX(date_division) AS sid FROM `%s`.`tb_reconsolidation_qpay` ORDER BY `date_division` DESC;", db_name), "sid");
			int min_date = MyDBUtil.getInstance().getInt(String.format("SELECT MIN(date_division) AS sid FROM `%s`.`tb_reconsolidation_qpay` ORDER BY `date_division` DESC;", db_name), "sid");
			MyDBUtil.getInstance().execute(String.format("INSERT INTO `%s`.`tb_consolidation_update_%s` SET `update_date`=now(), `file`='%s', `user_id`='%s', `type`='6'", db_name, db_name, f.getName().toString(), user_sid));
			...
			int i=0;
			try {
				//query.append(String.format("DELETE FROM `%s`.`tb_consolidation`;", db_name));
				for (Row row : sheet) {
					...
					if (isHeaderContained) {
						...
						for (Cell cell : row) {
							if(cell.getStringCellValue().toLowerCase().replaceAll("\n","_").replaceAll(" ", "_").replaceAll("__", "_").matches(".*branch_id.*")){	upload+=1 ;}
						...
						query.append(String.format("INSERT INTO `%s`.`tb_reconsolidation_qpay` SET", db_name));
						for (Cell cell : row) {
							if (columnsMap.get(cell.getColumnIndex()) != null) {
								...
								if (strTmp != null) {
									...
									case 0:
										try {
											if ((strTmp.toString().indexOf(":") > -1) && (strTmp.toString().indexOf("/") > -1) ) {	
									...
									default:
										...
										if ((strTmp.indexOf(":") > -1) && (strTmp.indexOf("/") > -1) ) {
											if(strTmp.substring(0, strTmp.indexOf("/")).length() < 2)	cellDate = strTmp.substring(strTmp.lastIndexOf("/")+1, strTmp.indexOf(" "))+ "0" +strTmp.substring(0, strTmp.indexOf("/"));
											else cellDate = strTmp.substring(strTmp.lastIndexOf("/")+1, strTmp.indexOf(" "))+ strTmp.substring(0, strTmp.indexOf("/"));
											
											query.append(String.format(" `date_division`='%s',", cellDate));
										}
										break;
									}
								}
				...
				query.append(String.format("DELETE FROM `%s`.`tb_reconsolidation_qpay` WHERE `update_sid` = %d AND `date_division` BETWEEN %s AND %s;", db_name, sid, min_date, max_date));
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (count > 0) {
