...
<%
		...
		try {
			...
			int max_date = MyDBUtil.getInstance().getInt(String.format("SELECT MAX(date_division) AS sid FROM `%s`.`tb_consolidate` ORDER BY `date_division` DESC;", db_name), "sid");
			int min_date = MyDBUtil.getInstance().getInt(String.format("SELECT MIN(date_division) AS sid FROM `%s`.`tb_consolidate` ORDER BY `date_division` DESC;", db_name), "sid");
			MyDBUtil.getInstance().execute(String.format("INSERT INTO `%s`.`tb_consolidation_update_%s` SET `update_date`=now(), `file`='%s', `user_id`='%s', `type`='1'", db_name, db_name, f.getName().toString(), user_sid));
			//MyDBUtil.execute(String.format("DELETE FROM `%s`.`tb_consolidate`;", db_name));
			...
			try {
				for (Row row : sheet) {
					...
					if (isHeaderContained) {
						...
						query.append(String.format("INSERT INTO `%s`.`tb_consolidate` SET", db_name));
						for (Cell cell : row) {
				...
				query.append(String.format("DELETE FROM `%s`.`tb_consolidate` WHERE `update_sid` = %d AND `date_division` BETWEEN %s AND %s;", db_name, sid, min_date, max_date));
				//query.append(String.format("DELETE FROM `%s`.`tb_consolidate` WHERE `door_name` IS NULL;", db_name));
				query.append(String.format("DELETE FROM `%s`.`tb_consolidate` WHERE `door_code` IS NULL OR `door_code` = '' OR `door_code` = '%Door Code';", db_name));
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (count > 0) {
