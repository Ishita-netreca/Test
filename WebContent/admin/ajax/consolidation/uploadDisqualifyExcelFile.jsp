...
<%
		...
		try {
			...
			int max_date = MyDBUtil.getInstance().getInt(String.format("SELECT MAX(date_division) AS sid FROM `%s`.`tb_reconsolidation_disqualified` ORDER BY `date_division` DESC;", db_name), "sid");
			int min_date = MyDBUtil.getInstance().getInt(String.format("SELECT MIN(date_division) AS sid FROM `%s`.`tb_reconsolidation_disqualified` ORDER BY `date_division` DESC;", db_name), "sid");
			MyDBUtil.getInstance().execute(String.format("INSERT INTO `%s`.`tb_consolidation_update_%s` SET `update_date`=now(), `file`='%s', `user_id`='%s', `type`='2'", db_name, db_name, f.getName().toString(), user_sid));
			...
			try {
				...
				for (Row row : sheet) {
					...
					if (isHeaderContained) {
						...
						for (Cell cell : row) {
							if(cell.getStringCellValue().toLowerCase().replaceAll("\n","_").replaceAll(" ", "_").replaceAll("__", "_").matches(".*door_code.*")){	upload+=1 ;}
							...
							if (strTmp != null) {
								...
								//if(strTmp.equals("Dealer Fee")) flag = "qpay";
								isHeaderContained = false;
							}
						}
						
					} else {
						query.append(String.format("INSERT INTO `%s`.`tb_reconsolidation_disqualified` SET", db_name));
						for (Cell cell : row) {
				...
				query.append(String.format("DELETE FROM `%s`.`tb_reconsolidation_disqualified` WHERE `update_sid` = %d AND `date_division` BETWEEN %s AND %s;", db_name, sid, min_date, max_date));
				query.append(String.format("DELETE FROM `%s`.`tb_reconsolidation_disqualified` WHERE `door_name` IS NULL OR `door_name` = '' OR `door_code` LIKE '%Door Name';", db_name));
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (count > 0) {
