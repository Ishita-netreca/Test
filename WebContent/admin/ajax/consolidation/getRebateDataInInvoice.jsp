...
<%
		...
		try {
			...
			query.append(String.format("SELECT abc.sid, abc.date, abc.name, abc.transaction_type, bc.*, abc.serial_no as esn FROM (SELECT tii.*, ti.`date` FROM `%s`.`tb_invoice_items_%s` AS tii LEFT JOIN `%s`.`tb_invoice_%s` AS ti ON tii.`invoice_no`=ti.`invoice_no`) AS abc LEFT JOIN", db_name, store_id, db_name, store_id));
			...
			query.append(String.format(" SELECT `sid` AS xid, `serial_no`,`promotion_item_sid`,`promotion_type` FROM `%s`.`tb_invoice_items_%s` WHERE `serial_no` IS NOT NULL AND `promotion_item_sid` IS NOT NULL", db_name, store_id));
			query.append(String.format(" AND `invoice_no` IN ( SELECT `invoice_no` FROM `%s`.`tb_invoice_%s` WHERE 1=1", db_name, store_id));
			...
			query.append(String.format(") AND `inventory_sid` IN (SELECT `sid` FROM `%s`.`tb_inventory_%s` WHERE `item_sid` IN (", db_name, store_id));
			query.append(String.format(" SELECT `sid` FROM `%s`.`tb_item_dict_%s` WHERE `item_type` IN ('0')", db_name, store_id));
			query.append(String.format(" ))) AS `a`"));
			query.append(String.format(" LEFT JOIN ("));
			query.append(String.format(" SELECT `serial_no`, SUM(`subtotal`) AS `total_primary_rate_plan_amount` FROM `%s`.`tb_invoice_items_%s` WHERE `serial_no` IS NOT NULL AND `item_type` IN ('2') AND `rateplan_type` IN ('0')", db_name, store_id));
			query.append(String.format(" AND `invoice_no` IN ( SELECT `invoice_no` FROM `%s`.`tb_invoice_%s` WHERE 1=1", db_name, store_id));
			if (search_start_date != null && search_start_date.length() > 0 && search_end_date != null && search_end_date.length() > 0) {
				query.append(String.format(" AND (`date` BETWEEN STR_TO_DATE('%s','%%m/%%d/%%Y') AND STR_TO_DATE('%s','%%m/%%d/%%Y') ) ", search_start_date, search_end_date));
			}			
			...
			query.append(String.format(" LEFT JOIN ("));
			query.append(String.format(" SELECT `serial_no`, SUM(`subtotal`) AS `total_features_data_plan_amount` FROM `%s`.`tb_invoice_items_%s` WHERE `serial_no` IS NOT NULL AND `item_type` IN ('2') AND `rateplan_type` IN ('1')", db_name, store_id));
			query.append(String.format(" AND `invoice_no` IN ( SELECT `invoice_no` FROM `%s`.`tb_invoice_%s` WHERE 1=1 ", db_name, store_id));
			if (search_start_date != null && search_start_date.length() > 0 && search_end_date != null && search_end_date.length() > 0) {
				query.append(String.format(" AND (`date` BETWEEN STR_TO_DATE('%s','%%m/%%d/%%Y') AND STR_TO_DATE('%s','%%m/%%d/%%Y') ) ", search_start_date, search_end_date));
			}	
			...
			query.append(String.format(" LEFT JOIN ("));
			query.append(String.format(" SELECT `promotion_item_sid`,`new_act_rebate`,`upg_rebate`,`port_in_rebate`,`sor_rebate`,`aal_rebate`,`aal_bogo_rebate`,`aal_pogo_rebate` FROM `%s`.`tb_rebate_items_%s`", db_name, store_id)); 
			...
			query.append(String.format("(SELECT * FROM `%s`.`tb_%s` WHERE 1=1", db_name, table));
