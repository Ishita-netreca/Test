...
<%
		...
		try {
			...
			int max_date = MyDBUtil.getInstance().getInt(String.format("SELECT MAX(date_division) AS sid FROM `%s`.`tb_consolidation` ORDER BY `date_division` DESC;", db_name), "sid");
			int min_date = MyDBUtil.getInstance().getInt(String.format("SELECT MIN(date_division) AS sid FROM `%s`.`tb_consolidation` ORDER BY `date_division` DESC;", db_name), "sid");
			...
			try {
				for (Row row : sheet) {
					...
					if (isHeaderContained) {
						...
						query.append(String.format("INSERT INTO `%s`.`tb_consolidation` SET", db_name));
						for (Cell cell : row) {
				...
				query.append(String.format("DELETE FROM `%s`.`tb_consolidation` WHERE `update_sid` = %d AND `date_division` BETWEEN %s AND %s;", db_name, sid, min_date, max_date));
				query.append(String.format("DELETE FROM `%s`.`tb_consolidation` WHERE `door_name` IS NULL OR `door_name` = '' OR `door_name` LIKE '%Door Name';", db_name));
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (count > 0) {
