...
<%
		...
		try {
			...
			// 쿼리 입력
			query.append(String.format("SELECT a.`file`, DATE_FORMAT(a.`update_date`, '%%m/%%d/%%Y %%H:%%i:%%s') as update_date, a.`user_id`, a.`sid`, a.`type`, b.`user_id` FROM `%s`.`tb_consolidation_update_%s` AS a LEFT JOIN `wrp`.`tb_user` AS b ON a.`user_id` = b.`sid`", db_name, db_name)); 
