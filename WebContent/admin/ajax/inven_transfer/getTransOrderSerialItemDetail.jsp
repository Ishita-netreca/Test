...
<%
	...
	try {
		...
		// 쿼리 입력
		query.append(String.format("SELECT DISTINCT `a`.*,`b`.`description` FROM (SELECT * FROM `%s`.`tb_inventory_trans_items_%s` WHERE `trans_sid`=%d AND `item_code`='%s') AS `a` ",db_name,db_name,transSid,item_code));
