...
<%
		...
		try {
			...
			query.append(String.format("SELECT `sid`,`type`,`from`,`to`,`commission_value` FROM `%s`.`tb_commission_%s` WHERE `profile_sid` IN ('%d') AND `target_type` IN ('%d') AND `target_sid` IN ('%d')", db_name, storeId, profileSid, target_type, target_sid));
