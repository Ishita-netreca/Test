	...
	function _printExpressPOItemsList(grid_id) {
		...
		if (elem) {
			...
			for (i =0, len = _expressAddedItems.length; i < len; i++) {
				...
				obj.sub_total = obj.item_cost * obj.pre_qty;
	...
	return {
		...
		getSerialNoListFromExcelFile: function(file_elem, print_element_id, print_str_format) {
	        ...
	        if (file_elem.files.length > 0) {
	            ...
	            if (ext !== ".xls") {
					WRPCommon.MsgBoxModule.alert({
						message: "You must select file : *.xls"
					});
	                return;
	            }
		...
		submitNonSerialItemDataInExpress: function(qty, grid_id) {
			...
			if (item === undefined) {
				WRPCommon.MsgBoxModule.alert({
					message: "There is no item you select"
				});
			}
		...
		setExpressPO: function(callback) {
			...
			for (i = 0, len = _expressAddedItems.length; i < len; i++) {
				obj = _expressAddedItems[i];
				if (obj.item_sid === undefined || obj.item_sid < 1) {
					WRPCommon.MsgBoxModule.alert({
						message: "Serial No Error. Please, try again"
					});
			...
			try {
				...
				for(i =0; i < _expressAddedItems.length; i++) {
					if(_expressAddedItems[i].item_type < 2) {
						if(parseInt(_expressAddedItems[i].pre_qty) !=  parseInt(_expressAddedItems[i].qty)) { 
							WRPCommon.MsgBoxModule.alert({
		...
		setItemCostInNewPOAddedItem: function(grid_id, item_sid, item_cost) {
			var item;
			...
			if (item_sid === undefined || item_cost === undefined || item_sid < 1 || item_cost < 0) {
				return;
			}
